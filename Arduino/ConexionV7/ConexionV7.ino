...
int pinDHT = 26;
DHTesp dht;
Adafruit_BMP085 bmp; //Esta libreria es para 085 y 180 (según dicen en el github de adafruit)
...
void setup() {
  ...
  // Inicializar el sensor BMP lo que sea
  if (!bmp.begin()) {
    Serial.println("Error: No se pudo encontrar el sensor BMP. Verifica el cableado.");
    while (1) {}
  }
...
void EnvioDatos() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Leer los datos del sensor BMP lo que sea 
    float presion = bmp.readPressure() / 100.0; // Convertir a hPa (heptopascales)
